...
Status: - [ ] (in progress: a canonical `benchmarks/r2_parity.py` gate landed; current baseline (n=1024) — `solve` 0.96×, `eigh` 0.98×, `dot` 2.13× at parity; `matmul` 0.66×, `add` 0.09×, `multiply` 0.84× below parity. Verified `matmul` hits OpenBLAS `cblas_dgemm` (not a naive loop); measured the residual is a **bundled-OpenBLAS build/tuning difference** vs NumPy's OpenBLAS (same thread count, still ~1.66× slower) — fixable only by rebuilding the bundled OpenBLAS. `add` is the lazy-materialization gap (reverted a routing fix that overran — see R2_CPU). See `documentation/dev/R2_PERF_FINDINGS.md`)
