...
Status: - [ ] (in progress: `benchmarks/r2_parity.py` gate landed; current baseline (n=1024) — **7/8 at parity** (`add` ~1.1×, `solve` ~0.96×, `invert` ~1.1×, `dot` ~2×, `multiply` ~0.93×, `determinant` ~1.24×, `eigh` ~0.98×). `matmul` hovers just under the bar (~0.87×): the raw `cblas_dgemm` is already faster than NumPy (0.0044s vs 0.0060s at 20 threads), the residual is per-call wrapper cost (fresh-result VirtualAlloc + `create_matrix`/MemoryGovernor + Python IO-planning). Landed in R2.2: (1) LAPACK determinant, (2) fair invert/determinant cache-clearing in the benchmark, (3) add-flakiness root-cause (stale-binary namespace merge — `native.py` now loads from `__file__`), (4) AVX2 f64 sub/mul/div + full-span hardening, (5) OpenBLAS 0.3.28 rebuilt DYNAMIC_ARCH+threaded, (6) `:memory:` backing switched to `VirtualAlloc` (the add/multiply win), (7) CPU dgemm no longer pins operands, (8) GEMM bumps to 20 threads. See `documentation/dev/R2_PERF_FINDINGS.md`)
