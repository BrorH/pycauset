...
M = pc.Matrix(data) # Returns IntegerMatrix

# Create an empty (zero-filled) 5x5 integer matrix
M_int = pc.Matrix(5, dtype=int)

# Create a dense boolean matrix
M_bool = pc.Matrix(1000, dtype=bool)
```

#### [[pycauset.CausalMatrix]]
For causal set theory, we often deal with causal matrices that are binary (0 or 1) and strictly upper triangular (representing the causal order). The `CausalMatrix` function is the standard way to create these. It returns a highly optimized [[pycauset.TriangularBitMatrix]].

```python
# Create a 1000x1000 causal matrix
# By default, it populates with random Bernoulli(0.5) data
C = pc.CausalMatrix(1000)
...
C_from_list = pc.CausalMatrix(data)
```
...
### Vectors
In addition to Matrices, `pycauset` supports efficient 1D Vectors ([[pycauset.FloatVector]], [[pycauset.IntegerVector]], [[pycauset.BitVector]]). See the [Vector Guide](Vector%20Guide.md) for details.
...
v_large = pc.Vector(1000000, dtype="float")
```
...
### Saving a Matrix
Matrices are backed by temporary storage (RAM or temporary files) that is released when the program exits. To permanently save a matrix, use [[pycauset.save]].
...
### Storage Management
`pycauset` uses a hybrid storage system to optimize performance and memory usage.

#### Memory Threshold
Small objects are stored entirely in RAM (using anonymous memory mapping) to avoid disk I/O overhead. Large objects are automatically backed by temporary files on disk.

The default threshold is **10 MB**. You can configure this using [[pycauset.set_memory_threshold]]:
...
#### Temporary Files
For objects larger than the memory threshold, `pycauset` manages backing files automatically. Files are stored in a `.pycauset` directory (or `$PYCAUSET_STORAGE_DIR`).
...
Use [[pycauset.matmul]](lhs, rhs) for true matrix multiplication (`numpy.matmul` semantics). The result is an [[pycauset.IntegerMatrix]] containing path counts / integer dot products.
...
### IntegerMatrix
The result of a multiplication is an [[pycauset.IntegerMatrix]]. It is read-only.
