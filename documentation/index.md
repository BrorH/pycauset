...
## How is pycauset?
Pycauset is written in C++ and stores all binary matrix entries as individual bits - saving orders of magnitude from using, say, [numpy](https://numpy.org/) for the same task. It employs a hybrid storage model: small matrices live in RAM for speed, while large matrices are automatically stored as memory-mapped binary files. This means there is no upper limit to how large $N$ can be - as long as you have the time and storage. For example, an external storage medium can be used for this purpose.
