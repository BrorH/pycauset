    ...
    std::pair<std::unique_ptr<DenseMatrix<double>>, std::unique_ptr<DenseMatrix<double>>> qr(const std::string& q_file = "", const std::string& r_file = "") const {
        if constexpr (!std::is_same_v<T, double>) {
            ...
            for (size_t k = 0; k < n_; ++k) {
                ...
                double norm_sq = 0.0;
                // We can not easily reduce a scalar in ParallelFor without atomic or thread-local accumulation.
                // For N=5000, a simple loop is fast enough for one column (5000 elements).
                // But let us try to use ParallelFor with chunks if needed.
                // Actually, for column-major access (which this is NOT, it is row-major), accessing column k is strided.
                // q_data[i * n_ + k]
