...
std::unique_ptr<MatrixBase> add(const MatrixBase& a, const MatrixBase& b, const std::string& result_file) {
    ...
    DataType res_dtype = ObjectFactory::resolve_result_type(type_a, type_b);
    if (res_dtype == DataType::BIT) res_dtype = DataType::INT32; // Promote BIT to INT32 for arithmetic
    MatrixType res_mtype = ObjectFactory::resolve_result_matrix_type(mtype_a, mtype_b);
...
std::unique_ptr<MatrixBase> outer_product(const VectorBase& a, const VectorBase& b, const std::string& result_file) {
    uint64_t rows = a.size();
    uint64_t cols = b.size();
    
    // Determine result type
    DataType res_dtype = ObjectFactory::resolve_result_type(a.get_data_type(), b.get_data_type());
    
    auto result = ObjectFactory::create_matrix(rows, res_dtype, MatrixType::DENSE_FLOAT, result_file);
...
std::unique_ptr<VectorBase> matrix_vector_multiply(const MatrixBase& m, const VectorBase& v, const std::string& result_file) {
    ...
    uint64_t n = m.rows();
    DataType res_dtype = ObjectFactory::resolve_result_type(m.get_data_type(), v.get_data_type());
