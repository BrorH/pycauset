...
def _normalize_dtype_token(token: Any) -> str | None:
    ...
    alias = {
        ...
        "identity": "float64",
        # DiagonalMatrix / SymmetricMatrix / AntiSymmetricMatrix are bound as
        # float64 in R1; they materialize to a full n-by-n float64 array on export.
        "diagonal": "float64",
        "symmetric": "float64",
...
def export_to_numpy(obj: Any, *, allow_huge: bool, dtype: Any = None, copy: bool = True) -> Any:
    ...
    # Try fast native export if available (only if copy is allowed/requested).
    # If copy=False, we prioritize the buffer protocol view (below), which avoids
    # the allocation performed by the native fast export.
    if copy:
